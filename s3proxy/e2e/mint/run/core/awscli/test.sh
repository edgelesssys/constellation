...
readonly HASH_1_KB=$(md5sum "${MINT_DATA_DIR}/datafile-1-kB" | awk '{print $1}')
readonly HASH_65_MB=$(md5sum "${MINT_DATA_DIR}/datafile-65-MB" | awk '{print $1}')
...
function test_copy_object() {
  ...
  # TODO(derpsteb): Remove once we implement copy-object and put the md5sum of the original object into a metadata field.
  # The expected hash should be the one that is returned by put-object.
  # If we use the hardcoded hash it won't match the etag returned by copy-object,
  # as copy-object works on encrypted data.
  hash_1_kb=$(echo "$out" | jq -r .ETag | sed -e 's/^"//' -e 's/"$//')
...
main() {
  ...
  test_create_bucket &&
    ...
    # TODO(deprsteb): This test is disabled for now as we don't support presigned URLs yet.
    # test_presigned_object &&
    test_upload_object_10 &&
    test_multipart_upload_10 &&
    test_bucket_lifecycle &&
    test_serverside_encryption &&
    # TODO(derpsteb): Reenable this test once we support Range on GetObject requests.
  ...
  # These are commented out on upstream.
  # TODO(derpsteb): Check if these can be run.
  # test_worm_bucket && \
  # test_legal_hold
