...
resource "azurerm_lb" "loadbalancer_ssh" {
  ...
  dynamic "frontend_ip_configuration" {
    ...
    content {
      name                          = "PrivateIPAddress"
      private_ip_address_allocation = "Dynamic"
      subnet_id                     = azurerm_subnet.loadbalancer_ssh_subnet[0].id
    }
  }
}
...
module "loadbalancer_backend_control_plane_ssh" {
  ...
  loadbalancer_id                = azurerm_lb.loadbalancer_ssh.id
  frontend_ip_configuration_name = azurerm_lb.loadbalancer_ssh.frontend_ip_configuration[0].name
...
module "loadbalancer_backend_worker_ssh" {
  ...
  loadbalancer_id                = azurerm_lb.loadbalancer_ssh.id
  frontend_ip_configuration_name = azurerm_lb.loadbalancer_ssh.frontend_ip_configuration[0].name
  ports                          = [{ name = "ssh-worker", port = "22", health_check_protocol = "Tcp", path = null, priority = 100 }]
}
...
resource "azurerm_subnet" "loadbalancer_subnet_ssh" {
  count                = var.emergency_ssh ? 1 : 0
  name                 = "${local.name}-lb-ssh"
  resource_group_name  = var.resource_group
  virtual_network_name = azurerm_virtual_network.network.name
  address_prefixes     = ["10.10.0.0/16"]
}
