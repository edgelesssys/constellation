...
func newRmCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "Remove an object from the versions API.",
		RunE:  runRm,
	...
	cmd.Flags().Bool("debug", false, "Whether to enable debug logging")
...
func runRm(cmd *cobra.Command, args []string) (retErr error) {
...
func (a *awsClient) deleteImage(ctx context.Context, ami string, region string, dryrun bool, log *logger.Logger) error {
	...
	_, err = a.ec2.DeregisterImage(ctx, &req)
