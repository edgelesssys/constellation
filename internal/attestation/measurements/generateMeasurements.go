...
func main() {
	...
	fmt.Printf("Generating measurements for %s\n", defaultConf.Image)
	...
	newFile := astutil.Apply(file, func(cursor *astutil.Cursor) bool {
		...
		if clause, ok := n.(*ast.CaseClause); ok && len(clause.List) > 0 && len(clause.Body) > 0 {
			...
			if provider == cloudprovider.QEMU {
				log.Println("Skipping QEMU since measurements cannot be autogenerated at this time")
				return true
			}
			log.Printf("pos: %d, returnpos: %d, results0pos: %d, lbracepos: %d, rbracepos: %d\n", returnStmt.Pos(), returnStmt.Return, returnStmt.Results[0].Pos(), returnStmt.Results[0].(*ast.CompositeLit).Lbrace, returnStmt.Results[0].(*ast.CompositeLit).Rbrace)
			elts := returnStmt.Results[0].(*ast.CompositeLit).Elts
			for i := range elts {
				log.Printf("elt[%d]: %d\n", i, elts[i].Pos())
			}
	...
	// fset.File(newFile.Pos()).SetLines(nil)
