...
func (a Client) uploadAzureSEVSNP(versions AzureSEVSNPVersion, versionNames []string, date time.Time) (res []putCmd, err error) {
...
func (a Client) UploadAzureSEVSNP(ctx context.Context, version AzureSEVSNPVersion, date time.Time) error {
	...
	dateStr := date.Format("2006-01-02-15-04") + ".json"
	err := apiclient.Update(ctx, a.s3Client, AzureSEVSNPVersionAPI{Version: dateStr, AzureSEVSNPVersion: version})
	if err != nil {
		return err
	}
...
func (a Client) DeleteList(ctx context.Context, attestation variant.Variant) error {
	if attestation.Equal(variant.AzureSEVSNP{}) {
		return apiclient.Update(ctx, a.s3Client, AzureSEVSNPVersionList{})
	}
	return fmt.Errorf("unsupported attestation type: %s", attestation)
}

func (a Client) deleteAzureSEVSNPVersion(versions AzureSEVSNPVersionList, versionStr string) (ops []crudOPNew, err error) {
...
func (a Client) addVersionToList(ctx context.Context, attestation variant.Variant, fname string) error {
	...
	return apiclient.Update(ctx, a.s3Client, AzureSEVSNPVersionList(versions))
}
