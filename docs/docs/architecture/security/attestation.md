...
Cluster-facing, Constellation's [_JoinService_](../components/microservices.md#joinservice) verifies each node joining the cluster given the configured ground truth runtime measurements.
...
By verifying the first node during the [initialization](../components/microservices.md#bootstrapper) and configuring the ground truth measurements that are subsequently enforced by the _JoinService_, the whole cluster is verified in a transitive way.
